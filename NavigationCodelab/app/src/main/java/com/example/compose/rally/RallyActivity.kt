...
fun RallyNavHost(navController: NavHostController, modifier: Modifier = Modifier) {
    NavHost(
        ...
        composable(
            ...
            // setup deep link. test using: adb shell am start -d "rally://accounts/Vacation" -a android.intent.action.VIEW
            deepLinks = listOf(navDeepLink {
                uriPattern = "rally://${RallyScreen.Accounts.name}/{name}"
            })
        ) { entry ->
            val accountName = entry.arguments?.getString("name")
            SingleAccountBody(account = UserData.getAccount(accountName = accountName))
        }
    }
}
